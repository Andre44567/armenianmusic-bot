...
def download_audio(query_or_url):
    ...
    source = query_or_url.strip() if is_url else f"ytsearch:{query_or_url}"
    title = "Անhайт"
    last_error = None
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if info:
                if "entries" in info and info["entries"]:
                    entry = info["entries"][0]
                    title = entry.get("title", "Անhайт") if entry else "Անhайт"
                else:
                    title = info.get("title", "Անhайт")

        for f in os.listdir(tmp_dir):
            fp = os.path.join(tmp_dir, f)
            if os.path.getsize(fp) > 1000:
                print(f"✅ Ներберռнверц: {f}")
                return fp, tmp_dir, title, None

        last_error = "Файл чстехцвец"
    except Exception as e:
        last_error = str(e)
        print(f"❌ download_audio ERROR: {e}")

    print("❌ Չстацвец. last_error:", last_error)
